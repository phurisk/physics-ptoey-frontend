...
import { Edit, Trash2, User as UserIcon, Tag, DollarSign, ImageOff, Star, Layers } from "lucide-react"
...
const STATUS_LABELS: Record<string, string> = { DRAFT: "ฉบับร่าง", PUBLISHED: "เผยแพร่", CLOSED: "ปิด", DELETED: "ถูกลบ" }
              ...
              {loading ? (
                ...
                courses.map((record) => (
                  <TableRow key={record.id} className={`group ${record.status === "DELETED" ? "opacity-60" : ""}`}>
                    ...
                    <TableCell className="text-sm text-gray-600">{record.subject ? getSubjectLabel(record.subject) : "-"}</TableCell>
                    <TableCell className="text-sm text-gray-600">{record.gradeLevel ? getGradeLevelLabel(record.gradeLevel) : "-"}</TableCell>
                    <TableCell className="sticky right-0 z-10 border-l border-gray-100 bg-white group-hover:bg-muted/50">
                      <div className="flex items-center justify-end gap-1">
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <Button variant="ghost" size="icon" className="text-purple-600" asChild>
                              <Link href={`/admin/courses/${record.id}/chapters`}>
                                <Layers className="h-4 w-4" />
                              </Link>
                            </Button>
                          </TooltipTrigger>
                          <TooltipContent>จัดการบทเรียน</TooltipContent>
                        </Tooltip>
                        <Tooltip>
                          <TooltipTrigger asChild>
