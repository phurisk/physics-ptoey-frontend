...
export default function AllBooksPage() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        const mapped = list.filter((c) => c?.isActive !== false).map((c: any, idx: number) => {
  ...
  const loadBooks = async (catId?: string) => {
    ...
    setError(null)
    try {
      const slug = categories.find((c) => c.id === (catId || selectedCategory))?.slug || ""
        ...
        <div className="mb-6 flex flex-wrap justify-center gap-3">
          {categories.map((c) => (
            <Button
              ...
              className={`px-5 rounded-full ${selectedCategory === c.id ? "bg-yellow-400 text-white" : ""}`}
            >
              {c.name}
            </Button>
          ))}
            ...
            {Array.from({ length: 8 }).map((_, i) => (
              <Card key={`sk-${i}`} className="overflow-hidden">
                <CardContent className="p-0">
                  <div className="aspect-[640/906] relative"><Skeleton className="absolute inset-0" /></div>
        ...
        {!loading && !error && (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {ebooks.map((book) => {
              ...
              const percent = hasDiscount ? calculateDiscount(book.price, book.discountPrice) : 0
              return (
                <Card key={book.id} className="overflow-hidden group">
                  <CardContent className="p-0">
                    <div className="relative aspect-[640/906] bg-white">
                      <Image
                        src={book.coverImageUrl || "/placeholder.svg?height=200&width=350"}
                        alt={book.title}
                        fill
                        className="object-contain group-hover:scale-105 transition-transform duration-300"
                      />
                      {hasDiscount && (
                        <Badge className="absolute top-3 left-3 bg-red-500 text-white">-{percent}%</Badge>
                      )}
                    </div>
                    <div className="p-4 space-y-3">
                      ...
                      <div className="flex items-baseline gap-2">
                        {hasDiscount ? (
                          <>
                            <div className="text-lg font-bold text-yellow-600">฿{book.discountPrice.toLocaleString()}</div>
                      ...
                      <div className="pt-1">
                        <Button className="bg-yellow-400 hover:bg-yellow-500 text-white w-full" onClick={() => handleBuy(book.id)}>
                          ซื้อเลย
                        </Button>
                      </div>
                    </div>
