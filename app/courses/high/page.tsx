...
export default function HighCoursesPage(){
  ...
  useEffect(()=>{let c=false;(async()=>{try{setLv(true);const p=new URLSearchParams({postType:"วิดีโอแนะนำ-ม.ปลาย",limit:"1"});const r=await fetch(`/api/posts?${p.toString()}`,{cache:"no-store"});const j=await r.json().catch(()=>({}));const list:Post[]=Array.isArray(j?.data)?j.data:Array.isArray(j)?j:[];const f=list[0];const u=firstUrl(f?.content||"");const e=u?(y(u)||v(u)):null;if(!c)setVideo(e)}finally{if(!c)setLv(false)}})();return()=>{c=true}},[])
  
  useEffect(()=>{let c=false;(async()=>{try{setLc(true);const r=await fetch(`/api/courses`,{cache:"no-store"});const j=await r.json().catch(()=>({}));const list:Course[]=Array.isArray(j?.data)?j.data:Array.isArray(j)?j:[];const fl=list.filter((x)=>(x as any)?.category?.name==="คอร์สแนะนำ-ม.ปลาย");if(!c)setCourses(fl)}finally{if(!c)setLc(false)}})();return()=>{c=true}},[])

  useEffect(()=>{let c=false;(async()=>{try{setLs(true);const p=new URLSearchParams({postType:"ภาพสรุป-ม.ปลาย"});const r=await fetch(`/api/posts?${p.toString()}`,{cache:"no-store"});const j=await r.json().catch(()=>({}));const list:any[]=Array.isArray(j?.data)?j.data:Array.isArray(j)?j:[];const m=list.map((p:any)=>({id:String(p?.id),desktop:p?.imageUrl||null,mobile:p?.imageUrlMobileMode||null,title:p?.title||""}));if(!c)setSums(m)}finally{if(!c)setLs(false)}})();return()=>{c=true}},[])

  const line="https://line.me/ti/p/@csw9917j"
  const summaryBanner = sums[0] || null

  return (
    <section className="min-h-screen bg-gradient-to-br from-white via-yellow-50/30 to-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="text-center mb-12">
          <h1 className="text-4xl lg:text-5xl font-bold text-gray-900 mb-4 text-balance">คอร์ส ม.ปลาย</h1>
          <p className="text-lg text-gray-600 max-w-2xl mx-auto text-pretty">วิดีโอแนะนำและคอร์สแนะนำสำหรับ ม.ปลาย</p>
        </div>
              ...
              <CardContent className="p-0">
                <div className="relative w-full aspect-[21/9] bg-white">
                  {summaryBanner?.desktop && (
                    <Image src={summaryBanner.desktop} alt={summaryBanner.title || 'summary'} fill className="object-contain hidden md:block" />
                  )}
                  {summaryBanner?.mobile && (
                    <Image src={summaryBanner.mobile} alt={summaryBanner.title || 'summary'} fill className="object-contain md:hidden" />
                  )}
        ...
        {lc ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {courses.slice(0, 3).map((c) => (
              <Card key={c.id} className="overflow-hidden group border-2 border-gray-100 hover:border-yellow-500/50 transition-all duration-300 hover:shadow-2xl">
                <CardContent className="p-0">
                  <div className="aspect-video relative bg-white overflow-hidden">
                    <Image src={c.coverImageUrl || "/placeholder.svg"} alt={c.title} fill className="object-cover group-hover:scale-110 transition-transform duration-500" />
                  ...
                  <div className="p-6 space-y-4 bg-white">
                    ...
                    <div className="flex items-center gap-2">
                      {c.isFree ? (
                        <span className="inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold bg-green-100 text-green-700">ฟรี</span>
                      ) : (
                        <span className="inline-flex items-center px-3 py-1 rounded-full text-sm font-bold bg-yellow-100 text-yellow-700">฿{Number(c.price||0).toLocaleString()}</span>
                      )}
                    </div>
        ...
        <div className="text-center mt-16">
          <div className="inline-flex flex-col items-center gap-4">
            ...
            <a href={line} target="_blank" rel="noopener noreferrer">
              <Button size="lg" className="bg-[#06C755] hover:bg-[#05b24c] text-white rounded-2xl px-10 py-4 font-bold text-lg shadow-2xl hover:shadow-3xl transition-all duration-300 transform hover:-translate-y-1 hover:scale-105">
